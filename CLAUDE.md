...
- Android Studio (Hedgehog 이상) + Android SDK 설치
- Java 17 이상 (`java -version` 확인)
...
```bash
# 1. Next.js 정적 빌드 + Capacitor 복사/동기화
npm run cap:copy

# 2. (플러그인 추가/변경 후) 네이티브 프로젝트 동기화
npm run cap:sync

# 3. Android Studio로 프로젝트 열기
npm run cap:open
# → Android Studio에서 Build > Build Bundle(s) / APK(s) > Build APK(s)
```

### packages/web 내에서 직접 빌드
...
| 파일                            | 역할                                     |
| ------------------------------- | ---------------------------------------- |
| `capacitor.config.ts`           | appId, appName, webDir 설정              |
| `packages/web/next.config.ts`   | `output: 'export'` (정적 export)         |
| `packages/web/lib/capacitor.ts` | 플랫폼 감지 + Preferences 래퍼           |
| `android/`                      | Capacitor 생성 Android 네이티브 프로젝트 |

자세한 빌드 가이드: `docs/apk-build-guide.md`
